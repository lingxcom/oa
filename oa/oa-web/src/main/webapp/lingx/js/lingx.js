	...
	if(methodOptions.type=='JSON'){
		if(methodOptions.currentRow){
				...
				Lingx.post("d",{c:"method_script",e:entityOptions.code,m:methodOptions.code,id:id},function(json){
					if(json.ret){
						if(methodOptions.fields.list.length==0){
							...
							openWindow(entityOptions.name+"-"+methodOptions.name,'e?e='+entityOptions.code+'&m='+methodOptions.code+'&'+(methodOptions.toField?methodOptions.toField:'id')+'='+id+condi,methodOptions.winStyle);
						}
					}else{
			...
			if(methodOptions.fields.list.length==0){
				...
				openWindow(entityOptions.name+"-"+methodOptions.name,'e?e='+entityOptions.code+'&m='+methodOptions.code+condi,methodOptions.winStyle);
			}
		}
		//end json
		...
		if(methodOptions.currentRow){
			...
			if(array&&array.length>0){
				var id=array[0].data.id;
				openWindow(entityOptions.name+"-"+methodOptions.name,'e?e='+entityOptions.code+'&m='+methodOptions.code+'&id='+id+condi,methodOptions.winStyle);
			}else{
				lgxInfo("请选择要操作的记录");
			}
		}else{
		...
		if(methodOptions.currentRow){
			...
			if(array&&array.length>0){
				var id=array[0].data.id;
				openWindow(methodOptions.name,'e?e='+entityOptions.code+'&m='+methodOptions.code+'&id='+id+condi,methodOptions.winStyle);
			}else{
				lgxInfo("请选择要操作的记录");
			}
		}else{
