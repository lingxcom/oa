...
function pushForm(obj){
	if(obj.inputType=="hidden"||obj.enabled==false){
	    ...
	    '  <textarea type="text" class="form-control" autocomplete="off"  id="'+obj.code+'"   />'+
	    '</div>'+
	  	'</div>';
		  //template=$(template);
		...
		var rvalue=$.type(obj.value)=="object"?obj.value.value:obj.value;
		$("#form").append('<input type="hidden" class="form-control" id="'+obj.code+'" value="'+rvalue+'">');
		var field=obj;
		if(field.value.exists){
			...
			field.value=$.type(obj.value)=="object"?field.value.text:obj.value;
		}
		var template='<div class="form-group '+(obj.formClass||"col-xs-10")+'">'+
	    '<label for="'+obj.code+'" class="'+(obj.formClass1||"col-xs-4")+' " style="line-height:32px;text-align:right;">'+(obj.isNotNull?"<span style='color:red'>*</span>":"")+obj.name+':</label>'+
	    '<div class="'+(obj.formClass2||"col-xs-8")+'">'+
	    '  <span  id="'+obj.code+'_F"  class="span-control" ></span>'+
	    '</div>'+
	  '</div>';
	  appendForm(template,(obj.formClass||"col-xs-10"))// $("#form").append(template);
	  $("#form").find("#"+obj.code+"_F").html(field.value);
	}else if(obj.inputType=="datefield"){
	
		var template='<div class="form-group '+(obj.formClass||"col-xs-10")+'">'+
		    '<label for="'+obj.code+'" class="'+(obj.formClass1||"col-xs-4")+' " style="line-height:32px;text-align:right;">'+(obj.isNotNull?"<span style='color:red'>*</span>":"")+obj.name+'</label>'+
		    '<div class="'+(obj.formClass2||"col-xs-8")+'">'+
		    '  <input type="text" class="form-control form_date" autocomplete="off"  data-date-format="yyyy-mm-dd" id="'+obj.code+'"  >'+
		    '</div>'+
		  '</div>';
		  appendForm(template,(obj.formClass||"col-xs-10"))// $("#form").append(template);
		  $("#form").find("#"+obj.code).val(obj.value);
		...
		var template='<div class="form-group '+(obj.formClass||"col-xs-10")+'">'+
		    '<label for="'+obj.code+'" class="'+(obj.formClass1||"col-xs-4")+' " style="line-height:32px;text-align:right;">'+(obj.isNotNull?"<span style='color:red'>*</span>":"")+obj.name+'</label>'+
		    '<div class="'+(obj.formClass2||"col-xs-8")+'">'+
		    '  <input type="text" class="form-control form_datetime" autocomplete="off"  data-date-format="yyyy-mm-dd hh:ii:ss" id="'+obj.code+'" >'+
		    '</div>'+
		  '</div>';
		  appendForm(template,(obj.formClass||"col-xs-10"))// $("#form").append(template);
		  $("#form").find("#"+obj.code).val(obj.value);
		...
		var template='<div class="form-group '+(obj.formClass||"col-xs-10")+'">'+
		    ...
		    '<input type="text" readonly style="background-color:#fff;" onclick="dialogUploadFile(\'文件中心\',\''+obj.code+'\',\''+(obj.inputOptions||"JSON")+'\')" class="form-control" id="'+obj.code+'_text"  >'+
		    '<span class="input-group-btn"><button class="btn btn-default" onclick="dialogUploadFile(\'文件中心\',\''+obj.code+'\',\''+(obj.inputOptions||"JSON")+'\')" type="button">选择...</button></span></div>'+
		...
		var template='<div class="form-group '+(obj.formClass||"col-xs-10")+'">'+
		  ...
		  $("#form").find("#"+obj.code).val(value);
		  $("#form").find("#"+obj.code+"_text").val(text);
	}else if(obj.inputType=="htmlview"){
		var template='<div class="form-group '+(obj.formClass||"col-xs-10")+'">'+
	    '<label for="'+obj.code+'" class="'+(obj.formClass1||"col-xs-4")+' " style="line-height:32px;text-align:right;">'+(obj.isNotNull?"<span style='color:red'>*</span>":"")+obj.name+'</label>'+
	    '<div class="'+(obj.formClass2||"col-xs-8")+'">'+
	    '  <div id="'+obj.code+'"  style="height:34px;line-height:34px;font-size:14px;">'+obj.value+'</div>'+
	    '</div>'+
	  '</div>';
	  appendForm(template,(obj.formClass||"col-xs-10"))// $("#form").append(template);
	}else{
		...
		var template='<div class="form-group '+(obj.formClass||"col-xs-10")+'">'+
		    '<label for="'+obj.code+'" class="'+(obj.formClass1||"col-xs-4")+' " style="line-height:32px;text-align:right;">'+(obj.isNotNull?"<span style='color:red'>*</span>":"")+obj.name+'</label>'+
		    '<div class="'+(obj.formClass2||"col-xs-8")+'">'+
		    '  <input type="text"  autocomplete="off"  class="form-control"  id="'+obj.code+'"  >'+
		    '</div>'+
		  '</div>';
...
<body style="background:#e2ece3;overflow-x:hidden;padding:0px;margin:0px;">
