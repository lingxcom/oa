...
function init(){
	...
	$.each(links,function(ind,obj){
		var el=$(obj);
		var url=el.attr("url");
		url=Lingx.urlAddParams(url,{instance_id:instanceId,LingxOpertor:model=="view"?"readonly":""});
		el.empty();
	...
	$.each(dialogoptions,function(ind,obj){
		...
		btn.bind("click",function(){
			openWindow("选择框",Lingx.urlAddParams("e?e="+etype+"&m=combogrid&cmpId="+id,{}));
		});
		if(el.prop("disabled")){
	...
	$.each(dialogtrees,function(ind,obj){
		...
		btn.bind("click",function(){
			openWindow("选择框",Lingx.urlAddParams("e?e="+etype+"&m=combotree&cmpId="+id,{}));
		});
		if(el.prop("disabled")){
			btn.prop("disabled",true);
			el.prop("disabled",false);
