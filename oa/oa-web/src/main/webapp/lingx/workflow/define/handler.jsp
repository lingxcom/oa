...
if("getDefineList".equals(cmd)){
	...
	case 1:
		name="开始";
	if(lingx.queryForInt("select count(*) from tlingx_wf_define_task where define_id=? and type=1",defineId)>0){
		ret.put("code", -1);
	...
	if(lingx.queryForInt("select count(*) from tlingx_wf_define_task where define_id=? and top=? and `left`=?",defineId,realTop,realLeft)>0){
		ret.put("code", -1);
	...
	if(lingx.queryForInt("select count(*) from tlingx_wf_define_line where define_id=? and source_id=? and target_id=? ",defineId,sid,tid)>0){
		ret.put("code", -1);
		out.println(JSON.toJSONString(ret));
		return;
	}
